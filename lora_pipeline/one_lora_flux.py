...
def _inject_flux_workflow_payload(
    wf: Dict[str, Any],
    *,
    seed: int,
    base_model_name: str,
    ...
    wf["38"]["inputs"]["unet_name"] = str(base_model_name)
...
async def run_comfy_batch_workflow(
    *,
    ...
    for sel_idx, pos_prompt in prompts_to_run:
        ...
        if all(str(node_id) in wf for node_id in ["31", "41", "43", "45", "38", "9"]):
            _inject_flux_workflow_payload(
                wf,
                seed=int(new_seed),
