...
def _inject_dual_flux_workflow_payload(
    wf: Dict[str, Any],
    *,
    seed: int,
    model_name: str,
    ...
    wf["38"]["inputs"]["unet_name"] = str(model_name)
    ...
    wf["43"]["inputs"]["t5xxl"] = str(negative_prompt) if negative_prompt else wf["43"]["inputs"]["clip_l"] 
    wf["45"]["inputs"]["lora_name"] = str(content_lora_name)
    wf["45"]["inputs"]["strength_model"] = float(strength_model)
    wf["45"]["inputs"]["strength_clip"] = float(strength_clip)
    wf["46"]["inputs"]["lora_name"] = str(style_lora_name)
    wf["46"]["inputs"]["strength_model"] = float(strength_model)
    wf["46"]["inputs"]["strength_clip"] = float(strength_clip)
    wf["9"]["inputs"]["filename_prefix"] = str(filename_prefix)
...
async def run_comfy_batch_workflow(
    *,
    ...
    for sel_idx, pos_prompt in prompts_to_run:
        ...
        new_seed = int.from_bytes(os.urandom(4), byteorder="big", signed=False) if gen_seed is None else int(gen_seed) + int(sel_idx)
        ...
        _inject_dual_flux_workflow_payload(
            wf,
            seed=int(new_seed),
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False, encoding="utf-8") as tf:
            ...
            temp_json = tf.name
        try:
            result = await asyncio.wait_for(kit.execute(temp_json), timeout=float(exec_timeout)) if exec_timeout and float(exec_timeout) > 0 else await kit.execute(temp_json)
        ...
        for img_idx, img_url in enumerate(images):
            ...
            if save_prompt_json:
                meta = {
                    ...
                    "seed": int(new_seed),
                    "workflow": wf_json,
                    "mode": "custom_workflow_dual_lora_flux",
                    "sel_idx": int(sel_idx),
                    "endpoint": url,
                }
