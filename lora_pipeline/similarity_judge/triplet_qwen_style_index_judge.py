...
#     "\n"
#     "判定目标：两张图是否属于同一种稳定画风/同一风格族。\n"
#     "你的判断必须基于整张图的全局主导风格，而不是局部区域、单个物体、局部纹理或某一小块细节的相似。\n"
#     "只有当风格相似性在图像的大部分主要视觉区域内都成立，且整体呈现出的风格机制一致时，才能判为风格一致。\n"
#     "\n"
#     "允许以下差异仍判为风格一致：\n"
#     "- 内容/主体/场景不同\n"
#     "- 构图与视角不同\n"
...
#     "\n"
#     "但你必须特别注意以下规则：\n"
#     "- 不能因为某一个局部区域（例如脸部、衣服、背景一角、某种纹理）风格相似，就判定整体风格一致\n"
#     "- 如果只有少数局部区域风格接近，而大部分区域在媒介感、线条体系、材质纹理、光影模型或整体调色策略上不一致，则必须判为不一致\n"
#     "- 只有当两张图在“全局主导风格”上保持一致，即风格机制覆盖了图像的大多数主要区域，才能输出一致\n"
#     "- 若两张图只是共享某些局部视觉元素、常见后期效果或个别颜色倾向，但整体风格统治力不同，则判为不一致\n"
#     "\n"
#     "只有当出现“风格机制”层面的明显变化才判不一致，例如：\n"
#     "- 真实摄影 vs 插画/渲染\n"
#     "- 线稿/勾线体系变化（有线稿→无，粗线→细线，漫画勾线→水彩边缘）\n"
...
#     "- 光影模型变化（硬边影视布光→柔和漫反射插画光→霓虹强对比等）\n"
#     "- 调色与色彩策略变化（低饱和复古→高饱和糖果色→黑白素描等）\n"
#     "- 全局主导风格不一致：例如仅局部区域相似，但整张图的大部分区域体现出不同的渲染与风格机制\n"
#     "\n"
#     "输出规则：你只能输出一个字符：0 或 1。\n"
#     "1 = 画风高度一致（同一风格族，且这种风格在整张图的大部分主要区域内稳定成立）；0 = 画风不一致。\n"
#     "不要输出任何多余文字、空格、换行或标点。"
# )
...
#     "请仅从“画风 / 视觉风格”角度比较图片A与图片B，忽略人物/物体身份、动作含义、故事语义与场景类别。\n"
#     "\n"
#     "请综合以下维度做判断，并采用“宽松一致性”标准：只要核心风格机制在全局范围内一致，即使主体、构图、视角、细节密度不同，也可以判为一致。\n"
#     "但你的判断必须建立在整张图的全局主导风格上，而不是某个局部区域的相似。\n"
#     "如果风格相似只体现在少数局部、小面积区域、单个对象或局部纹理上，而不能代表整张图的大部分主要视觉区域，则不能判为一致。\n"
#     "\n"
#     "重点维度（更高权重）：\n"
#     "1) 媒介与渲染方式：摄影/3D/插画/水彩/油画/厚涂/赛璐璐/像素/素描 等\n"
#     "2) 笔触与线条体系：是否有线稿、线条粗细/抖动、边缘处理、笔触颗粒\n"
...
#     "\n"
#     "全局一致性要求（非常重要）：\n"
#     "6) 以上重点维度的一致性应当体现在图像的大部分主要视觉区域，而非只出现在局部小区域\n"
#     "7) 先判断每张图的“全局主导风格”是什么，再比较两者是否一致；不要被某个局部相似纹理或某个物体误导\n"
#     "8) 如果两张图只是局部风格相似，但整体呈现的媒介感、渲染范式、线条体系、材质生成方式或调色策略不同，输出 0\n"
#     "9) 只有当风格机制在整体上占主导，并且覆盖图像的大多数主要区域时，才输出 1\n"
#     "\n"
#     "次要维度（允许变化）：\n"
#     "10) 构图与视角：机位、镜头感、取景范围不同不应直接判为不一致\n"
#     "\n"
#     "判定：\n"
#     "- 若多数“重点维度”一致，且这种一致性体现在全局主导风格与大面积区域上，输出 1。\n"
#     "- 若只有局部区域相似、而整体风格分布不一致，输出 0。\n"
#     "- 只要出现明显的风格机制改变（如摄影↔插画、线稿体系突变、材质/渲染范式突变、整体调色策略完全不同），输出 0。\n"
#     "\n"
#     "最终只输出一个字符：0 或 1。"
# )
