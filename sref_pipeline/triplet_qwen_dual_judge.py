...
def _process_one_name(name: str) -> Tuple[Dict[str, Any], int]:
    ...
    for cdir in content_dirs:
        ...
        decision, detail = judge_pair_voting(
    ...
    return rec, 1 if dual_pass else 0

def _worker_process_main(model: str, base_url: str, task_queue: mp.Queue, result_queue: mp.Queue):
...
def main():
    ...
    ap.add_argument("--root", required=True, help="输出目录根：包含 style_and_content/ content_*/ style_*/")
    ap.add_argument("--num_samples", type=int, default=2000, help="随机抽样数量（<=0 表示全量）")
    ap.add_argument("--seed", type=int, default=1234)
    ap.add_argument("--conf_thr", type=float, default=0.5, help="单次有效阈值")
    ...
    if endpoints:
        ...
        for model_name, url in endpoints:
            for _ in range(per):
                p = mp.Process(target=_worker_process_main, args=(model_name, url, task_queue, result_queue))
                p.daemon = False
                p.start()
                workers.append(p)
        for _ in workers:
        ...
        with mp.Pool(processes=procs) as pool:
            for rec, ok_inc in tqdm(
                pool.imap_unordered(_process_one_name, picked),
                total=len(picked),
                desc="DualJudge-MP",
                unit="img",
            ):
        ...
        for i, name in enumerate(picked, 1):
            ...
            results.append(rec)
            dual_ok_cnt += ok_inc
            log(f"[{i}/{len(picked)}] {name} -> Content:{rec['content_pass']} ({rec['content_ratio']:.2f}), Style:{rec['style_pass']} ({rec['style_ratio']:.2f}) -> Dual:{rec['dual_pass']}")
            _update_maps_and_flush(rec)

    log(f"[DONE] Processed {len(picked)} samples.")
    log(f"Content Pass: {content_ok_cnt} ({content_ok_cnt/len(picked):.2%})")
    log(f"Style Pass:   {style_ok_cnt} ({style_ok_cnt/len(picked):.2%})")
    log(f"Dual Pass:    {dual_ok_cnt} ({dual_ok_cnt/len(picked):.2%})")
