...
def set_lora(
    ...
    """
    Applies LoRA (Low-Rank Adaptation) processors to specified attention blocks in a Flux model.

    Args:
        model (Flux): The model to modify.
        lora_rank (int): The rank for the LoRA processors.
        double_blocks_indices (list[int] | None, optional): Indices of double stream blocks to apply LoRA to. Defaults to all double blocks if None.
        single_blocks_indices (list[int] | None, optional): Indices of single stream blocks to apply LoRA to. Defaults to all single blocks if None.
        device (str | torch.device, optional): Device on which to initialize LoRA processors. Defaults to "cpu".

    Returns:
        Flux: The model with updated attention processors.
    """
