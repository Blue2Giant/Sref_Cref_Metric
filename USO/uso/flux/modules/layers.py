...
from ..math import attention, rope, attention_hotmap
...
class SingleStreamBlockLoraProcessor_hotmap(SingleStreamBlockLoraProcessor):
    def __init__(
        self, dim: int, rank: int = 4, network_alpha=None, lora_weight: float = 1
    ):
        """
        return a hotmap of attention weights for interpretability
        """
        super().__init__(dim = dim, rank=rank, network_alpha=network_alpha, lora_weight=lora_weight)

    def forward(self, attn: nn.Module, x: Tensor, vec: Tensor, pe: Tensor) -> Tensor:
        # print('using single steam block---->',x.shape)
        mod, _ = attn.modulation(vec)
        x_mod = (1 + mod.scale) * attn.pre_norm(x) + mod.shift
        qkv, mlp = torch.split(
            attn.linear1(x_mod), [3 * attn.hidden_size, attn.mlp_hidden_dim], dim=-1
        ...
        #得到img token的起始点
        # self.img_token_length = x.shape[1]
        # self.vec = vec

        # compute attention
        attn_1, hotmap = attention_hotmap(q, k, v, pe=pe)
        self.hotmap = hotmap.detach().cpu()  # save the hotmap for later use
...
class DoubleStreamBlockLoraProcessor_hotmap(DoubleStreamBlockLoraProcessor):
    def __init__(self, dim: int, rank=4, network_alpha=None, lora_weight=1):
        super().__init__(dim=dim, rank=rank, network_alpha=network_alpha, lora_weight=lora_weight)
    ...
    def forward(self, attn, img, txt, vec, pe, **attention_kwargs):
        ...
        #得到img token的起始点
        # self.txt_token_length = txt.shape[1]
        # self.img_token_length = img_v.shape[2]
        # self.vec = vec

        # attn1 = attention(q, k, v, pe=pe)
        attn1, hotmap = attention_hotmap(q, k, v, pe=pe)
        self.hotmap = hotmap.detach().cpu()  # save the hotmap for later use
        txt_attn, img_attn = attn1[:, : txt.shape[1]], attn1[:, txt.shape[1] :]
